    ...
    return (
        <div className="statusBar d-flex flex-column w-100 px-1 my-2">
            ...
            <div className={`statusBar__bar d-flex align-items-center ${small ? 'statusBar__bar-small' : ''}`}>
                <span className={`statusBar__bar-line ${small ? 'statusBar__bar-line-small' : ''}`} style={{ width: `${state * 100 / max}%` }}></span>
                <span className={`statusBar__bar-state text-light ${small ? 'statusBar__bar-state-small' : ''}`}>{state}</span>
            </div>
        </div>
    )
}
